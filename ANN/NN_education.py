...
# train_insts_filename, train_label_filename, test_insts_filename, test_label_filename = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
...
def load(filename):
    ...
    with file(filename, "r") as fin:
        ...
        for line in fin:
            line = line.strip()
            segments = line.split(",")
            inst = [float(s) for s in segments]
            insts.append(inst)
...
def backward(preds, labels, (x, h1, h2)):
    n = preds.shape[0]
    e2 = preds - labels
...
for i in xrange(num_iters):
    h1, h2 = forward(train_insts)
    backward(h2, train_labels, (train_insts, h1, h2))
    # Gradient update.
    hiddenw -= lr * dhiddenw
    classw -= lr * dclassw
    classb -= lr * dclassb
    # Compute the error squared.
    squared_err = np.sum(np.square(h2 - train_labels))
    print squared_err
print "TRAINING COMPLETED! NOW PREDICTING."
end_time = time.time()
# Compute test set error.
_, preds = forward(test_insts)
